...
beta=c(-0.5,1,1,1,rep(0,p-3)) #O1/O2
#beta=c(-0.5,rep(sqrt(3/s_m),s_m),rep(0,p-s_m)) #O1'
alpha=rep(0,p+1) #O1/O1'
#alpha=c(0,1,1,1,rep(0,p-3)) #O2
#gamma=rep(0,p+1) #P1
gamma=c(0,1,rep(0,p-1)) #P2
gamma=c(0,rep(1/sqrt(s_pi),s_pi),rep(0,p-s_pi)) #P2'
...
num_mest=3;num_piest=3;num_totest=num_mest*num_piest+2
...
results_par <- foreach (t=1:tmax, .combine=rbind, .packages=c("glmnet")) %dopar% {
  ...
  for (j in 1:nrow(grid)){
    x=as.numeric(grid[j,])
    psi_mu=pred_m[x[1],]+R*(Y-pred_m[x[1],])/pred_pi[x[2],]-pred_theta_t[x[1]+3*(x[2]-1)]
