...
num_mest=5;num_piest=4;num_totest=num_mest*num_piest+2
...
results_par <- foreach (t=1:tmax, .combine=rbind, .packages=c("grf","glmnet","KRLS")) %dopar% {
  ...
  for (k in 1:K){
    ...
    fit_delta=glm(delta_nk~.,data=data.frame(x=X[-index,]),family=binomial)
    pred_pi_1=sum(delta_nk*R_nk)/sum(delta_nk);pred_pi_0=sum((1-delta_nk)*R_nk)/sum(1-delta_nk)
    pred_p_delta[index]=predict(fit_delta,newdata=data.frame(x=X[index,]),type="response")
  ...
  for (j in 1:nrow(grid)){
    ...
    if (x[2]==1){
      ...
      w1=mean(pred_p_delta*(pred_m[x[1],]-Y)/pred_pi[x[2],])
      w2=mean((1-pred_p_delta)*(pred_m[x[1],]-Y)/pred_pi[x[2],])
      IF_pi=(delta*R/mean(delta)-pred_pi_1)*w1+((1-delta)*R/mean(1-delta)-pred_pi_1)*w2
      var_mod_t=c(var_mod_t,mean((psi_mu+IF_pi)^2)) #stratified
    }
  }
